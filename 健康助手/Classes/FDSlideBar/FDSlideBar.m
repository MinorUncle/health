...
- (instancetype)init {
    CGRect frame = CGRectMake(0, 0, DEVICE_WIDTH, 24);
...
- (void)setupItems {
    for (FDSlideBarItem* i in _items) {
    ...
    for (NSString *title in _itemsTitle) {
        ...
        item.frame = CGRectMake(itemX, 0, itemW, CGRectGetHeight(self.frame));
    ...
    self.contentSize = CGSizeMake(itemX, CGRectGetHeight(self.frame));
    ...
    CGRect rect = CGRectMake(0, self.frame.size.height - SLIDER_VIEW_HEIGHT, firstItem.frame.size.width, SLIDER_VIEW_HEIGHT);
...
- (void)scrollToVisibleItem:(FDSlideBarItem *)item {
    ...
    if (CGRectGetMinX(item.frame) > offset.x && CGRectGetMaxX(item.frame) < (offset.x + CGRectGetWidth(self.frame))) {
        return;
    }
//    offset.x = CGRectGetMaxX(item.frame) - _scrollView.bounds.size.width;
//
    ...
    if (selectedItemIndex < visibleItemIndex) {
        ...
        if (CGRectGetMaxX(_selectedItem.frame) < offset.x) {
            ...
            offset.x = CGRectGetMaxX(item.frame) - CGRectGetWidth(self.frame);
        }
    } else {
        ...
        if (CGRectGetMinX(_selectedItem.frame) > (offset.x + CGRectGetWidth(self.frame))) {
            offset.x = CGRectGetMaxX(item.frame) - CGRectGetWidth(self.frame);
        } else {
            offset.x = CGRectGetMinX(item.frame);
        }
...
//    _sliderView.layer.bounds = rect;
    ...
    rect.origin.x = item.frame.origin.x;
    rect.size.width = item.bounds.size.width;
    [_sliderView setFrame:rect];
